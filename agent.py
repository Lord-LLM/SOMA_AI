...
try:
    ...
    print("This is expected if you haven't created all agent files yet.")
    agents_loaded = False
...
if agents_loaded:
    ...
    # Fallback: Create a simple root agent without sub-agents for testing
    print("Creating basic root agent for testing...")
    root_agent = LlmAgent(
        name="StudyCompanionCoordinator",
        model=Gemini(model="gemini-2.5-flash-lite", retry_options=retry_config),
        description="AI Study Companion (Basic Mode - Sub-agents not loaded)",
        instruction="""You are a helpful study assistant. 
        You can help with learning, but some advanced features require sub-agents to be configured.
        ...
        Please create the individual agent files to enable full functionality."""
    )